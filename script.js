...
music.volume = 0.2; // 20% du volume
document.getElementById("profile-block").classList.add("show");

const profileBlock = document.getElementById("profile-block");

profileBlock.style.transformStyle = "preserve-3d";
profileBlock.style.transformOrigin = "center center";

profileBlock.addEventListener("mousemove", (e) => {
  const rect = profileBlock.getBoundingClientRect();
  const x = e.clientX - rect.left - rect.width / 2;
  const y = e.clientY - rect.top - rect.height / 2;
 const tiltY = (x / rect.width) * 45; // penche dans le bon sens
const tiltX = -(y / rect.height) * 45; // inclinaison verticale
